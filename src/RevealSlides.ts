...
export class RevealSlides {
  ...
  private extractRevealConfiguration(
    ...
    return {
      ...
      themeCss: asciidocAttributes.revealJsTheme
        ? `libs/reveal.js/theme/${asciidocAttributes.revealJsTheme}.css`
        : undefined,
  ...
  private getSlideIdUnderCursor(asciidocText: string, lineNumber: number) {
    ...
    try {
      ...
      const indexOfSectionAfterCursor = sections.findIndex(
        (section: SlideSection) => section.getLineNumber() > lineInAsciidoc,
      );
  ...
  private refreshReferenceToMyEditor() {
    const freshReferenceToBaseEditor = vscode.window.visibleTextEditors.find(
      (e) => e.document.uri === this.baseEditor.document.uri,
    );
